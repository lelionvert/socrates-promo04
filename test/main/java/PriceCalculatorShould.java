...
public class PriceCalculatorShould {
    ...
    public void computeCompletePrice() {
        double price = priceCalculator.calculatePrice(Accommodation.DOUBLE, CheckInDeadline.BEFORE_FIRST_MEAL, Checkout.AFTER_LAST_MEAL);
        assertThat(price).isEqualTo(510);
    }

    @Test
    public void computePriceWithoutFirstMeal() {
        double price = priceCalculator.calculatePrice(Accommodation.TRIPLE, CheckInDeadline.AFTER_FIRST_MEAL, Checkout.AFTER_LAST_MEAL);
        assertThat(price).isEqualTo(370);
    }

    @Test
    public void computePriceWithoutLastMeal() {
        double price = priceCalculator.calculatePrice(Accommodation.NONE, CheckInDeadline.BEFORE_FIRST_MEAL, Checkout.BEFORE_LAST_MEAL);
        assertThat(price).isEqualTo(200);
    }

    @Test
    public void computePriceWithoutFirstAndLastMeal() {
        double price = priceCalculator.calculatePrice(Accommodation.SINGLE, CheckInDeadline.AFTER_FIRST_MEAL, Checkout.BEFORE_LAST_MEAL);
